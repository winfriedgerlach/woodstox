...
public class MdibJTest {

    static final String PARTICIPANT_NS = "http://standards.ieee.org/downloads/11073/11073-10207-2017/participant";

    @Test
    public void testParseMdib() throws XMLStreamException, URISyntaxException, IOException {

        XMLInputFactory xmlInputFactory = XMLInputFactory.newInstance();
        WstxSAXParserFactory saxParserFactory = new WstxSAXParserFactory();
        byte[] mdibBytes = Files.readAllBytes(Paths.get(getClass().getResource("/winne/mdib.xml").toURI()));

        xmlInputFactory.setProperty(XMLInputFactory.IS_COALESCING, true);
        xmlInputFactory.setProperty(XMLInputFactory.IS_NAMESPACE_AWARE, true);
        saxParserFactory.setNamespaceAware(true);

        XMLEventReader xmlEventReader = xmlInputFactory.createXMLEventReader(new ByteArrayInputStream(mdibBytes));
